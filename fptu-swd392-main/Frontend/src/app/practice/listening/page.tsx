...
const MOCK_LISTENING_EXERCISES = [
  {
    id: "l1",
  ...
  },
  {
    ...
    duration: 20,
    totalQuestions: 10,
    description: "Listen to an academic lecture and answer comprehension questions",
    audioUrl: "/audio/sample.mp3",
  },
  {
    id: "l3",
    ...
    duration: 25,
    totalQuestions: 12,
    description: "Listen to a news report and answer detailed questions",
    audioUrl: "/audio/sample.mp3",
  },
];
...
export default function ListeningPracticePage() {
  ...
  const skillData = SKILLS_DATA.listening;
  const allExercises = MOCK_LISTENING_EXERCISES;
  ...
  const recentActivity = recentResults
    ...
    .map((result) => ({
      ...result,
      title: result.testTitle || "Listening Exercise",
    }));

  const completedIds = new Set(
    recentResults.filter((r) => r.skill === "listening").map((r) => r.testId),
  );
  ...
  return (
    <div className="min-h-screen py-12 bg-background">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Header */}
        <SkillHeader skill="listening" skillData={skillData} />

        {/* Statistics */}
        <ProgressBar
          skill="listening"
          completed={listeningProgress.testsCompleted}
          total={allExercises.length}
          currentLevel={
            listeningProgress.averageScore >= 80
              ? "C1"
              : listeningProgress.averageScore >= 60
                ? "B2"
                : "B1"
          }
            ...
            {recommendations.length > 0 ? (
              <div className="space-y-3">
                {recommendations.map((rec) => (
                  <Link
                    key={rec.id}
                    href={`/practice/listening/${rec.id}`}
